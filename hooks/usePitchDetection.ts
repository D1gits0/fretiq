...
// â”€â”€â”€ Tuning config â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// to change â€” all mapping logic below derives from these values.
//
// MIDI note numbers: C4 = 60, so E2 = 40, A2 = 45, D3 = 50, G3 = 55, B3 = 59, E4 = 64
const TUNING_MIDI: readonly number[] = [40, 45, 50, 55, 59, 64];

// â”€â”€â”€ String model â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const FFT_SIZE    = 2048; // must match useKatanaAudio â€” time-domain buffer size

const CLARITY_BANDS: ReadonlyArray<{ maxHz: number; threshold: number }> = [
  { maxHz: 200,      threshold: 0.90 },
  { maxHz: 500,      threshold: 0.86 },
  { maxHz: 800,      threshold: 0.76 },  // relaxed from 0.80 â€” B3/E4 on clean preset
  { maxHz: Infinity, threshold: 0.68 },  // relaxed from 0.72 â€” high frets on E4
];
...
// Must mirror train.py extract_features() exactly â€” same band boundaries,
...
// â”€â”€ Mel filterbank â€” built once at module load, never per-frame â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/**
 * Build a Float32Array of shape (N_MELS Ã— N_BINS) â€” row-major.
 * Matches train.py _build_mel_filterbank exactly:
 *   fft_freqs = linspace(0, sr/2, n_bins)   â† n_bins points, NOT n_bins+1
...
function _buildMelFilterbank(): Float32Array {
  ...
  // n_mels+2 evenly-spaced mel points â†’ Hz
  ...
  // Build (N_MELS Ã— N_BINS) filter matrix, row-major
...
/**
 * Build a Float32Array of shape (N_MFCC Ã— N_MELS) â€” row-major.
...
// Pre-computed â€” allocated once at module load
const _MEL_FILTERBANK = _buildMelFilterbank(); // Float32Array, shape (40 Ã— 1024)
const _DCT_MATRIX     = _buildDctMatrix();     // Float32Array, shape (13 Ã— 40)
 ...
 *   0â€“7   : band energies (normalised by total energy)
 ...
 *   12    : peak bin value (normalised to [0,1], bins 0â€“255)
 *   13â€“25 : 13 MFCCs (mel filterbank â†’ log â†’ DCT-II â†’ /20 â†’ clip[-1,1])
 */
function extractFeatures(bins: Uint8Array): number[] {
  ...
  // â”€â”€ Band energies (0â€“7) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Spectral centroid (8) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  let centroid = 0;
  if (totalEnergy > 0) {
  ...
  // â”€â”€ Spectral rolloff at 85% (9) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  let rolloffIdx = 0;
  if (totalEnergy > 0) {
  ...
  // â”€â”€ Spectral flatness (10) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Peak bin (11, 12) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  let peakIdx = 0;
  let peakVal = 0;
  for (let i = 0; i < N; i++) {
  ...
  // â”€â”€ MFCCs (13â€“25) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // Step 1: apply mel filterbank â€” (N_MELS Ã— N_BINS) @ bins â†’ (N_MELS,)
  ...
  // Step 2: log compression â€” matches train.py: log(mel_energy + 1e-6)
  const logMel = new Float32Array(_N_MELS);
  for (let m = 0; m < _N_MELS; m++) {
    logMel[m] = Math.log(melEnergies[m] + 1e-6);
  }

  // Step 3: DCT-II â€” (N_MFCC Ã— N_MELS) @ logMel â†’ (N_MFCC,)
  ...
  // Step 4: normalise â€” matches train.py: clip(mfccs / 20.0, -1, 1)
...
// â”€â”€â”€ Heuristic inference (fallback when model is not loaded) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Manual model loader â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
//   dense/kernel   [26, 128]  â†’ 3328 values â†’ 13312 bytes  @ offset 0
//   dense/bias     [128]      â†’  128 values â†’   512 bytes  @ offset 13312
//   dense_1/kernel [128, 32]  â†’ 4096 values â†’ 16384 bytes  @ offset 13824
//   dense_1/bias   [32]       â†’   32 values â†’   128 bytes  @ offset 30208
//   dense_2/kernel [32, 6]    â†’  192 values â†’   768 bytes  @ offset 30336
//   dense_2/bias   [6]        â†’    6 values â†’    24 bytes  @ offset 31104
//   Total: 31128 bytes
...
async function loadStringClassifier(
  ...
  try {
    ...
    // Build the model architecture â€” must match train.py build_model() exactly
    ...
    // Warm up â€” first predict is slow due to graph compilation
...
export function usePitchDetection(getAnalyser: () => AnalyserNode | null) {
  ...
  // â”€â”€ Load string classifier on mount â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  useEffect(() => {
    let cancelled = false;
    (async () => {
  ...
  // â”€â”€ Per-frame pitch detection loop â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  useEffect(() => {
    const tick = () => {
      ...
      // â”€â”€ Per-frame model debug log â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      // Runs every valid pitch frame â€” before debounce or note lock â€” so the
      ...
      // Note lock â€” don't switch notes until the committed one has been absent
      if (
        committedMidiRef.current !== -1 &&
      ...
      // â”€â”€ String inference â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      ...
      if (model) {
        // Model path â€” read freq-domain data and extract 26 features
        analyser.getByteFrequencyData(freqBufRef.current as Uint8Array<ArrayBuffer>);

        // â”€â”€ Latency profiler â€” logs avg after 200 frames, then stops â”€â”€â”€â”€â”€
        ...
        if (!latencyProfilerRef.current.done) {
          ...
          if (p.samples.length >= 50) {
            ...
            console.log(
              `[Fretiq latency] feature extraction + model.predict â€” 200 frames\n` +
      ...
      // â”€â”€ String stability gate â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      ...
      // before switching â€” prevents bouncing while a note rings.
      const framesNeeded = committedStringRef.current >= 0 &&
                           committedStringRef.current !== inferredString
        ? 6  // switching away from a committed string â€” higher bar
        : 4; // initial commit â€” lower bar
